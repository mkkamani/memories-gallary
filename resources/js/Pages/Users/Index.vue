    ...
    <AuthenticatedLayout>
        <div class="animate-fade-in text-foreground space-y-6">
            ...
            <Modal :show="showModal" @close="closeModal" max-width="md">
                <div class="mx-auto w-full max-h-[88vh] overflow-y-auto rounded-2xl border border-border bg-bg-card p-4 sm:p-6">
                    ...
                    <div class="mt-6 flex flex-col-reverse gap-2 sm:mt-8 sm:flex-row sm:justify-end sm:gap-3">
                        <button type="button" @click="closeModal" class="h-11 w-full sm:w-auto px-6 rounded-pill text-sm font-bold text-foreground hover:bg-bg-hover transition-all">Cancel</button>
                        <button type="button" @click="submit" :disabled="form.processing" class="flex w-full sm:w-auto items-center justify-center gap-2 h-11 px-8 rounded-pill bg-primary text-primary-foreground text-sm font-bold shadow-lg shadow-primary/20 hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-50 disabled:cursor-not-allowed">
